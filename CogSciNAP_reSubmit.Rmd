...
abstract: |
  *Sonority* is a fundamental notion in phonetics and phonology, central to many descriptions of the syllable and evidently useful as a predictor of phonotactics.
  Although widely-accepted, sonority lacks a clear basis in speech articulation or perception, and traditional sonority principles, 
  which have been exclusively based on discrete and symbolic machinery, and were not designed to be compatible with cognitive capacities, exhibit systematic gaps in empirical coverage.
  ...
  We present perception experiments that test our two NAP-based models against four traditional sonority models and we use a Bayesian data analysis approach to test and compare them. Our symbolic model outperforms all the other models we test, while our continuous bottom-up model manages to reach the second place, along the best tradtional models.
...
Our synergy of proposals has many advantages over traditional sonority accounts, including methodological aspects, theoretical perspectives, and, essentially, a better empirical coverage. In Subsection \@ref(sec:discussion2) we interpret the results of the main confirmatory experiments and in Section \@ref(sec:genDiscussion) we address some relevant implications, where we discuss the division of labor between sonority and other phonotactic factors, demonstrated with a holistic account of the phenomenon of /s/-stop clusters (Subsection \@ref(sec:division)). Finally, we also discuss the debate about the universality of sonority in light of our work (Subsection \@ref(sec:projection)), before we present our concluisons in Section \@ref(sec:conclusions).

<!-- usefulness of the top-down vs. bottom-up distinction instead of the common use of the phonology--phonetics dichotomy to express the distinction between discrete and continuous aspects of linguistic sound systems (Subsection \@ref(sec:dichotomies)). -->
...
In the remainder of this Introduction, we briefly present the relevant background on traditional sonority hierarchies and sonority principles (Subsection \@ref(sec:sonback)), and on the tension between discrete and symbolic entities on the one hand, and contious dynamic events on the other hand, in liguistic models of phonology (Subsection \@ref(sec:lingMod)).
...
Out of the 29 different cluster types in Table \@ref(tab:targetlist), more clusters occur regularly in Modern Hebrew [see @asherov2019syllable]. These include all of the eight sibilant-initial clusters, /sp, ʃp, sf ʃf, sm, ʃm, zm, zv/, and the liquid-second clusters, /fl, vl/. The voiceless cluster /ft/ and the /m/-initil clusters /ml, mn/ are marginally atttested in Modern Hebrew [@asherov2019syllable 75, 86].
...
(ref:participantsGer) Experiment 2: participants' age, gender, and education.
...
(ref:participantsHeb) Experiment 3: participants' age, gender, and education.
...
The first effort, which took place in 2019, collected data from student volunteers in Tel Aviv University and The Hebrew University of Jerusalem. The second data collection effort occurred already during the early phases of the global COVID-19 pandemic, which resulted in many ad-hoc locations and more heterogenous population in some respects. 
...
*NAP~td~* appears to exhibit some redundancy, as suggested by the relatively large degrees of overlap between some of the predictive distributions of the model. This is apparent from the overlap between violins in the left side (worse-formed) of the model fit with Eperiment 2 (left plot), and between violins in the right side (better-formed) of the model fit with Eperiment 3 (right plot) in Figure \@ref(fig:NAPtdFit).
...
A bird's eye view of all the six models in Experiment 2 is availale in Figure \@ref(fig:SonFitAll). The results of the model comparison in Experiment 2 are available in Table \@ref(tab:resultsmodels). It reveals a very clear advantage of *NAP~td~* over all the other models. 
The difference in elpd scores from the next three models---*SSP/MSD~col~* and *NAP~bu~*---is about 6 standrd errors (considering that the difference is around 90 elpd and the corresponding standard error is around 15), a very robust lead for *NAP~td~*. 
...
*NAP~td~* alone contributes the lion's share with 65% and *NAP~bu~* comes second with 14%. This is notable as both NAP models are essentially based on the same principle, lending support to the idea top-down and bottom-uo lead to two complementary models.
...
A bird's eye view of all the six models in Experiment 3 is availale in Figure \@ref(fig:SonFitAllHeb). 
The results of the model comparison of Experiment 3 (see Table \@ref(tab:resultsmodelsHeb)) reveal a borderline advantage of *NAP~td~* over all the other models.
The difference in elpd scores from the next two models---*MSD~col~* and *NAP~bu~*---is only about 2 standrd errors (considering the difference at around 10 elpd and the corresponding standard error at around 5),
...
The smaller success of the continuous *NAP~bu~* model is, in fact, still quite impressive. *NAP~bu~*, as it was implemented here, analyzing well-formedness directly from the continuous acoustic signal, is already as good as the best traditional models, and it still has many potential paths for improvement. For example, 
<!-- As a novel measurement of the acoustic signal, *NAP~bu~*'s partial success is, in fact, quite impressive: it still has a big room for improvement and it is already as good as the best traditional models.  -->
*NAP~bu~* can directly enjoy from independent improvements in extraction of periodic energy in digital signal processing, or from improvements in the conception of competition, and the formulas used to estimate or simulate it. We therefore interpret the results of *NAP~bu~* as very promising.
...
Note that Experiment 3 exhibits most of the general trends found in Experiment 2, albeit in a less compelling way. We suspect that this is due to the varios sources of noise that were introduced in the process. These include the smaller group of participants and the diverse physical locations for the administration of Experiment 3, which were mentioned in Subsection \@ref(sec:participants3). The results confirm this with a wider standard error for the intercepts of the different subjets in the statistical models in Experiemnt 3. On top of that, it was the impression of the experimenter (the first author) that Hebrew speakers in this study tended to have a less robust understanding of the notion of the syllable such that their performance may exhibit higher degerees of misunderstanding of the the meta-linguistic syllable count task. This impression seems consistent with the overall smaller range of response times in Experiment 3, whereby Hebrew speakers tended to respond relatively fast to ill-formed structures. 
...
(ii) in Subsection \@ref(sec:projection) we discuss the universality of sonority, with special attention to the body of works by Iris Berent and her colleagues, as well as to the works of the physicist and theoretical biologist, Howard Pattee (Subsection \@ref(sec:compofmind)).
...
###   /s/-stop clusters are well-formed
...
In a related work, Anonymous (in press) tests sonority models on diachronic corpus data of Modern Hebrew, which uses general phonemic trascriptions. In such common cases, when lingustic data is already abstracted into stable forms, only the symbolic NAP model is of relevance, and, once again, *NAP~td* outperformed traditional SSP-based accounts.  
