...
See Appendix \@ref(sec:considerationlist) for a list of considerations and criteria that we used in constructing the experimental stimulus set, and see the full stimulus set in Table \@ref(tab:targetlist).
...
<!-- The fact that none of the models failed in the estimation results shown above was used to fine-tune and settle on our data collection and data derivation techniques (see details in Appendix \@ref(sec:procedures)) and to finalize our model implementations (section \@ref(sec:modelimp)), with emphasis on our two novel NAP-based models. Importantly, the results of the following confirmatory study (Experiment 2) are consistent with those of Experiment 1. -->
...
While the model fits give us an insight into the behaviour of each model with respect to the data, they are not well-suited to compare the different models against a similar criterion. To do that, we run out-of-sample predictions using cross-validation to test each model's ability to predict unseen items.
...
(iii) some interesting differences between *col* and *exp* sonority hierarchies were observed, where the advantages of the minimal *col* sonority hierarchy proved more effective; 
...
A simplified conclusion that may illustrate this state of affairs would be to claim that the sonority intercept of onset clusters appears to be more impactful than the slope in determining syllabic well-formedness.
