...
<!-- --- -->
<!-- title             : "The double life of language systems: Modelling sonority with complementary symbol-based and signal-based models" -->
<!-- shorttitle        : "The double life of language systems" -->
<!-- --- -->
<!-- title             : "The double life of the Nucleus Attraction Principle: Modelling sonority with complementary symbol-based and signal-based models" -->
<!-- shorttitle        : "The double life of the Nucleus Attraction Principle" -->
...
<!-- In contrast to sonority hierarchies that tend to be flexible to a certain extent, the  small family of sonority principles has seen very little change over the years.  -->
...
<!-- , in seminal works such as @zwicky1972notesk; @hankamer1974sonority; @hooper1976introduction; @kiparsky1979metrical; @lowenstamm1981maximal; @steriade1982greek; @cairns1982markedness; @selkirk1984majorsk; @harris1983syllable; @mohanan1986theory, and @clements1990role. -->
...
<!-- experiments (section \@ref(sec:experiments)), and we propose a full account of /s/-stop clusters, which illustrates the division of labor between sonority and other phonotactic principles in the general discussion (Subsection \@ref(sec:division)). -->
...
Tones are used to distinguish words while tunes are 
<!-- which is  -->
used to demarcate units as well as to express non-propositional meanings (e.g., discourse-pragmatic intention, emotional state, socio-indexical identity, and attitudinal stance). 
The importance of pitch to human communication cannot be overstated.
...
Top-down perception inferences in our framework are different from bottom-up inferences in that top-down inferences access categorical representations, where speech is analyzable in terms of discrete symbols. Top-down inferences are thus limited to predictions that are based on the distributional probability of the recognized symbols, given their (constantly updated) history in the system.
...
In this paper we do not explore the statistical nature of top-down inferences. Instead, we operationalize the rationale behind NAP with symbolic machinery to present what we refer to as the top-down model of NAP. It is therefore important to note that  
our model implementation for the top-down version of NAP is not an attempt to model top-down cognitive processes.
<!-- the discrete arithmetic operations that we present in the top-down version of NAP are by no means assumed to model cognitive processes. -->
<!-- , given that top-down processes in perception are supposed to be limited to probabilistic inferences in our framework.  -->
...
<!-- Note that quantities that are derivable from regions of continuous curves can employ one of three general strategies---summing, averaging or extraction of peaks (local minima and maxima). These three measurement strategies differ greatly in the contribution they delegate to the duration dimension. -->
...
allows the accumulation of the individual measurements, keeping the contribution of the duration component in the final calculation. Summing is therefore essentially different from averaging (as well as from peak extraction) in that it is capable of uncovering the quantitative difference between two periodic sounds that have the exact same amplitude envelope yet differ in duration.
...
The distance between *CoM~syl~* and *CoM~ons~* is indicative of the amount of displacement of energy away from the center of the syllable, which we interpret as reflective of the degree of nucleus competition (see Figure \@ref(fig:com-4examples-1)).
<!-- [^cf-loess] -->
...
<!-- Also note that all  -->
Note that all
the media and original code that was used in this study are available at the following link on *Open Science Framework* (anonymized link): https://tinyurl.com/y4gvobyp 
...
Only the single-vowel productions were considered as targets, and an
<!-- We recorded accuracy and response time results from native German-speaking participants.  -->
<!-- An  -->
accurate response to our targets is always the monosyllabic option, yet we use the term "accuracy" to describe participants' responses only in a technical sense, without any assumption about correctness of responses. 
<!-- Participants were presented with a collection of speech items that were produced with one or two vowels, systematically for each combination of consonants in our set.  -->
...
monosyl_plot_abs <- 
  ...
  geom_text(aes(x=pos_mid,y=17.5,label=text), size=5, family = "Times", check_overlap=T) + #family = "Charis SIL",
  #
  facet_wrap(~syl, ncol=5) +
  theme(text = element_text(family = "Times"), panel.background = element_blank(), axis.title = element_text(size = 12), axis.text.y = element_blank(), axis.ticks = element_blank(), axis.text.x = element_text(size = 8), strip.text = element_blank())
  # theme(panel.background = element_blank(), axis.title = element_text(size = 12, family="Times"), axis.text.y = element_blank(), axis.ticks = element_blank(), axis.text.x = element_text(size = 8, family="Times"), strip.text = element_blank())
...
<!-- For these models, we assume that all the differences between adjacent categories were a-priori the same, that is, the prior for the distance between scores is a uniform Dirichlet distribution. -->
...
<!-- (\@ref(sec:division)) we discuss a potential phonotactic division of labor, demonstrated with a more holistic account of /s/-stop clusters; -->
(ii) in Subsection \@ref(sec:dichotomies) we discuss a potential phonotactic division of labor, demonstrated with a more holistic account of /s/-stop clusters.
<!-- \@ref(sec:dichotomies) we discuss the complementarity of discrete and continuous modes in cognitive modeling, suggesting that the top-down--bottom-up distinction exhibits a better fit with the discrete--continuous dichotomy than the classic phonetics--phonology dichotomy.  -->
...
*	Lastly, we avoid sequences of obstruents that differ in voicing due to the strong cross-linguistic tendency of obstruent clusters to agree in voicing (although note that German allows /ʃv/ and /t͡sv/ clusters while banning /ʃf/).
...
in order to maintain consistent prosody. Carrier sentences were also designed to minimize potential effects of resyllabification as well as co-articulation by controlling the segmental makeup immediately preceding and following target words (the Hebrew sentences used in this study are availale at the OSF repository (see in the opening of Section \@ref(sec:experiments)). 
...
<!-- Since all stimuli share the form /CCal/ where the rime *al* is predictable (this becomes clear to participants during the training session),  -->
...
<!-- , using ‘F' and ‘J' buttons respectively. The relevant buttons were covered with salient red-on-white ‘1' and ‘2' stickers, the rest of the laptop's keys are all white-on-black by design -->
...
[^cf-angle]: A slightly similar calculation can be found in Fullwood's [-@fullwood2014perceptual] *Sonority Angle*, although on different grounds, for different purposes, and with different outcomes.
