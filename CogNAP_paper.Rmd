...
<!-- Finally,  -->
We present our concluisons in Section \@ref(sec:conclusions) and we provide the technical details on the procedures used in the experiments in Section \@ref(sec:methods).
<!-- . Section \@ref(sec:methods) (*Methods*) provides technical details on the procedures used in the experiments. -->
...
However, in order to model the top-down contribution of sonority to the well-formedness of syllables, our NAP model should be also able to operate on abstract symbolic entities, like consonants and vowels. Therefore, the mechanisms that should be available to symbol-based processes need to qualitatively differ from the dynamic models that are available to bottom-up signal-based processes
<!-- the dynamic modes of bottom-up continuous signals  -->
and instead be based on the distributional probability of recognized symbols, given their history in the system.

The above description of top-down inferences, which are detached from the functional aspects of the bottom-up route, echo models of the language user as a *statistical learner*
<!-- [see, e.g., @christiansen1999power; @frisch2001psychologicalsk; @tremblay2013processing; @roettger2019evidential]  -->
and, more specifically, they are very much in line with models of *phonotactic learners*.[^cf-phonlearn]
<!-- [e.g., @coleman1997stochastic; @vitevitch2004webbasedsk; @bailey2001determinants; @hayes2008maximum; @hayes2011interpreting; @albright2009feature; @daland2011explaining; @futrell2017generative; @jarosz2017inputsk; @mayer2019phonotactic; @mirea2019usingsk]. -->
...
<!-- It is therefore important to note that our model implementation for the top-down version of NAP is not an attempt to directly reflect top-down cognitive processes. -->
...
The results of model stacking based on our experimental data (see Table \@ref(tab:modelstacking)) support this move, showing that the combined contribution of the two NAP models is relatively complementary (65% + 14%), and, when taken together, they make 79% of the combined contribution of all the six sonority models to a maximized elpd score, reflecting the combined ability of the models to predict unseen forms (see Subsection \@ref(sec:ModelComparison2)).
...
in order to maintain consistent prosody. Carrier sentences were also designed to minimize potential effects of resyllabification as well as co-articulation by controlling the segmental makeup immediately preceding and following target words
<!-- , see example in (\@ref(ex:sentence))  -->
(the full list of Hebrew sentences used in this study are availale at the OSF repository, see link in the opening of Section \@ref(sec:experiments)).
