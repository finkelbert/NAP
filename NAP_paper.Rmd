...
bibliography      : ["r-references.bib", "methods.bib", "phon.bib", "phon_sk.bib"]
...
knitr::include_graphics(rep(c("NAP_CCV.png","NAP_npV.png")))
```
...
other_scores <- read_tsv(file="CCal_model_predictions_fix.tsv", 
                           col_types = cols(
                               syl = col_character(),
                               SSP = col_integer(),
                               SSP_obs = col_integer(),
...
data_pilot <- read_list_opensesame(files_pilot, speaker="AA") %>%
    filter(corrRT > 100)
...
```{r, mpilot, include=FALSE}
 ...
 if(file.exists("m_pilot.RDS")){
     m_pilot <- readRDS("m_pilot.RDS")
 } else {
     m_pilot <- run_brms(data_pilot)
     saveRDS(m_pilot, file = "m_pilot.RDS")
 }
 if(file.exists("kfold_pilot.RDS")){
     
     kfold_pilot <- readRDS("kfold_pilot.RDS")
 } else {
     kfold_pilot <- map(m_pilot$models, kfold, folds = "stratified", group = "subj", K = 10)
     saveRDS(kfold_pilot, file = "kfold_pilot.RDS")
 }
...
```{r, include=FALSE}
opensesame_german <- "data/german_exp"

files_german <-
    list.files(path=opensesame_german,pattern="*.csv",full.names = TRUE)

data_german <- read_list_opensesame(files_german, speaker="AA") %>%
    filter(corrRT > 100)
...
if(file.exists("m_german.RDS")){
    m_german <- readRDS("m_german.RDS")
 } else {
     m_german <- run_brms(data_german, iter=4000, warmup=2000)
     saveRDS(m_german, file = "m_german.RDS")
 }
# 
```
```{r loo-models, include=FALSE}
if(file.exists("loo_german.RDS")){
    loo_german <- readRDS("loo_german.RDS")
    kfold_german <- readRDS("kfold_german.RDS")
} else {
     loo_german <- map(m_german$models, loo, pointwise = TRUE)
     saveRDS(loo_german, file = "loo_german.RDS")
     
     kfold_german <- map(m_german$models, kfold, folds = "stratified", group = "subj", K = 15)
     saveRDS(kfold_german, file = "kfold_german.RDS")
    }
```
...
In contrast,  *NAP~bu~* is modeled with a continuous predictor which is assumed to have a linear relationship with the log-response times. Finally, as baseline, we fitted a "null" model which assumes no relationship between the stimuli and the response times. All the models, included a random intercept and slope by subjects (except for the null model that included only a random intercept) and  the following weakly regularizing priors: $Normal(0, 10)$ for the intercept and $Normal(0, 1)$ for the rest of the parameters.

We evaluate the models in three different ways: (i) estimation, (ii) descriptive adequacy, and (iii) model comparison.

### (i) Estimation {-}
We report mean estimates and 95\% quantile-based Bayesian credible intervals. A 95\% Bayesian credible interval has the following interpretation: it is an interval containing the true value with 95% probability [see, for example, @Jaynes1976; @MoreyEtAl2015]. 
...
## pilot
subjects_pilot <- read.csv("subjects_pilot.csv") #%>% select(-X) %>% distinct(subject_nr, .keep_all = TRUE)
...
## main
subjects_main <- read.csv("subjects_main.csv") #%>% select(-X) %>% distinct(subject_nr, .keep_all = TRUE)
...
predictions$null %>%
  mutate(NAP= round(NAP_bu,7)) %>%
    ...
    geom_point(data= data_RT, aes(y = corrRT, x= factor(NAP)), color = "red3", alpha = .7, size =1.5) +
...
predictions$SSP_obs %>%
  ...
  filter(!is.na(SSP_obs)) %>%
   ...
   { ggplot(., aes(x = SSP_obs, y= pred) ) +
    ...
    geom_point(data=distinct(.,corrRT,stimulus,SSP_obs), aes(y = corrRT), color = "red3", alpha = .7, size =1.5) +
...
predictions$SSP %>%
  ...
  filter(!is.na(SSP)) %>%
   ...
   { ggplot(., aes(x = SSP, y= pred) ) +
    ...
    geom_point(data=distinct(.,corrRT,stimulus,SSP), aes(y = corrRT), color = "red3", size =1.5, alpha = .7) +
...
predictions$MSD_obs %>%
  ...
  filter(!is.na(MSD_obs)) %>%
   ...
   { ggplot(., aes(x = MSD_obs, y= pred) ) +
    ...
    geom_point(data=distinct(.,corrRT,stimulus,MSD_obs), aes(y = corrRT), color = "red3", size =1.5, alpha = .7) +
...
predictions$MSD %>%
  ...
  filter(!is.na(MSD)) %>%
   ...
   { ggplot(., aes(x = MSD, y= pred) ) +
    ...
    geom_point(data=distinct(.,corrRT,stimulus,MSD), aes(y = corrRT), color = "red3", size =1.5, alpha = .7) +
...
predictions$NAP_td %>% 
  ...
  { ggplot(., aes(x = NAP_td, y= pred) ) +
    ...
    geom_point(data=distinct(.,corrRT,stimulus,NAP_td), aes(y = corrRT), color = "red3", size =1.5)+
...
comparison <- loo::compare(x=kfold_german)
...
comparison <- loo::compare(x=kfold_classic)
...
comparison <- loo::compare(x=kfold_ordinal)
...
```{r SonFitAll,fig.cap = "(ref:SonFitAll)", warning=FALSE, fig.width=c(3.5,3.5,3.5,3.5,3.2,3.2), fig.asp=c(.5,.5,.5,.5,.5,.5), fig.show="hold"}
# , fig.align='center', fig.ncol=2
# ```{r SonFitAll,fig.cap = "(ref:SonFitAll)", warning=FALSE, out.width=c('40%','40%','40%','40%'), out.height=c('70%','70%','70%','70%'), fig.align='center', fig.show="hold"}
# ```{r SonFitAll,fig.cap = "(ref:SonFitAll)", warning=FALSE, fig.width=c(3,3,3,3), fig.asp=c(.4,.4,.4,.4), fig.show="hold"}
# ```{r SonFitAll,fig.cap = "(ref:SonFitAll)", warning=FALSE, fig.width=8, fig.asp=.4}
